...
| 自建 API 云存档 / 账号 | 可以（见 [`P-ACCOUNT`](../任务/P-ACCOUNT.md)） | **多数不允许或需书面批准**；独家更严 |
...
- **不是**上传游戏、不是签分成合同、不是过审上线。
- 通过后，才会进入后续：对接 SDK、上传构建、平台预览、正式提交（任务 `T-017`）。
...
| 字段 | 什么意思 | Fangrush 建议 |
|------|----------|---------------|
| What's your name? | 联系人真名 | 本人英文名或拼音全名 |
| What's your email? | 商务联系邮箱 | 长期会查、以后能收合同的邮箱 |
| What's the name of your studio or team? | 工作室/团队对外名 | **`Fangrush Studio`**（一人也可；勿写「狼与绵羊」） |
| In which country are you based? | 团队所在国家 | `China`（或实际常驻国） |
| How would you describe your studio or team? | 团队类型（多选） | Indie / Solo / Small indie；有 First-time 也勾；勿装大厂 |
| If you've previously released games… | 以前发过哪些平台 | 第一次就 **None / Not yet**，勿编造 |
| Can you provide us with links… | 作品链接（必填） | 优先 `https://fangrush.com`；粘贴块见发行目录照抄文件 |
| Which genres… | 常做品类 | `Strategy, Puzzle, Board game, HTML5 casual` |
| Which engines… | 常用引擎 | `HTML5 / TypeScript / Next.js (custom board-game engine)`；**不要写 Unity** |
| What are you looking to gain… | 期望从 Poki 得到什么 | Reach / Distribution、Ads monetization、Publishing support |
...
4. 字段级粘贴文本与提交勾选：[`distribution/poki/early-access-form.md`](../../distribution/poki/early-access-form.md)。
...
3. **工程准备**：门户用 `pnpm build:portal`（已关 Admin）；真接平台 SDK 等有号/邀约。
