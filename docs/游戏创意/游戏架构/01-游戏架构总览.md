...
> 工程红线见 [`01-工程契约与红线`](./01-工程契约与红线.md)；页面 / SEO 见 [`06`](./06-页面地图与SEO-GEO.md)；玩法意图见 [`docs/游戏创意/`](../游戏创意/00-文档索引.md)。  
...
| 共性层 | 本游戏落点 | 细则 |
|--------|------------|------|
| 规则 + 局面 + AI + 存档/皮肤 + Adapter + Admin | game-core / web 壳 | [01 工程契约与红线](./01-工程契约与红线.md) |
...
| L3 + L4 | `apps/player-web` + `apps/admin`（独立 Next.js 应用） |
| 跨应用契约 | `packages/web-shared` |
| 皮肤像素 | `packages/web-assets/public/skins/`（构建时同步到玩家端） |

边界：core **零 DOM**；web **不写吃子/胜负**。细则见 [01](./01-工程契约与红线.md)。
...
- 本地玩家端使用 `5003`，Admin 使用 `5002`；core 作为 workspace 依赖编进前端，**不单独起进程**。
- 规则与 AI **在浏览器**跑，不上 Server Route（见 [01](./01-工程契约与红线.md)）。
...
- `pnpm build:standalone` → 玩家端独立站 Next 产物；`pnpm build:admin` → Admin 独立产物。
...
| 渠道 | 产物 | 相对独立站的差异 |
|------|------|------------------|
| 独立站（P0） | `apps/player-web` 独立部署（如 Vercel） | 无广告演示；Admin 不进入玩家端产物 |
| 门户 H5（P2） | `build:poki` / `build:crazygames` | 关 Admin；广告走对应门户 SDK；默认零自建遥测 |
| App（预留） | Capacitor 包 Web | 仅留端口，未落地 |

原则：**一套 core + 玩家端壳 + Admin 壳**；渠道差在 shell / Adapter / 构建环境，**规则不分叉**。
...
同一份源码 (main)
        │
        ├─ pnpm build:standalone   → 玩家端独立站 → Vercel 等
        ├─ pnpm build:admin        → Admin 独立项目 → Vercel（可后置）
        ├─ pnpm build:poki         → Poki H5（关 Admin、门户广告）
        ├─ pnpm build:crazygames   → CrazyGames H5（关 Admin、门户广告）
...
| 门户 H5 | `pnpm build:poki` / `pnpm build:crazygames` | 对应门户广告 Adapter | Admin、独立站 AdSense、账号云存档 |
| App（预留） | 以后加 `build:app` / CI；Capacitor 包同一 Web | 商店广告 / IAP Adapter | 门户 SDK；勿当第三条业务分支 |

**登录**：只做独立站（见 [`P-ACCOUNT`](../../任务/独立站专属需求/P-ACCOUNT.md)）；门户不做云账号。App 若以后要登录，复用独立站账号能力，不要给门户再开一套。

**变量怎么防呆**（人只选「打哪种包」，每种包的配方由脚本/CI 写死）：

| 做法 | 作用 |
|------|------|
| 构建脚本定稿（`apps/player-web/scripts/build-target.mjs`） | 门户强制 `NEXT_PUBLIC_APP_SHELL=portal`、`ADMIN_ENABLED=false` |
...
关键变量类别（玩家端当前键名见 `apps/player-web/.env.example`，细则见 [01](./01-工程契约与红线.md)）：
...
```text
game-core     ← 永远一套
apps/player-web UI ← 玩家入口；按 shell 区分 standalone / portal
