...
> 状态：核心能力与 Admin 闭环已完成，包括候选运行、报告管理、证据播放、人工接管和反例归档。
>
...
- [x] 支持候选验收报告 JSON 导入、校验、本地持久化和导出。
- [x] 在 `/admin/levels` 展示日期化 11/13/0 摘要基线、`pass/review/reject` 筛选、命中规则和证据种子。
- [x] 证据种子链接将关卡、AI 档位和 seed 传入 `/admin/ai`。
- [x] 直接加载导入报告中的 mixed 完整行动序列，从真实 opening 逐步调用正式规则；规则不一致时显示具体失败步骤。
- [x] 从棋谱任意非终局帧通过一次性交接转入 AI 台，保持完整局面并自动开启狼方人工接管。
- [x] 载入或编辑候选 JSON，通过受 Admin 权限约束的服务端调用验收器；浏览器主线程不运行模拟。
- [x] review/reject 自动归档候选配置、完整报告和证据，最多保留 100 条；支持 JSON 导出和逐条删除。

候选工具不会写入正式 `LEVELS`。任何生产配置变更仍必须由代码修改、测试和代码审查完成，因此 review/reject 不能从 Admin 被直接发布。
