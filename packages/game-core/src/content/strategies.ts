...
export const WOLF_STRATEGIES: WolfStrategy[] = [
  ...
  S('three_wolf_surround', 'Three-wolf surround', '三狼包围', 'Two wolves restrict routes while the third converts the capture.', '两只狼限制路线，第三只狼完成捕食。', 'Two wolves can cover exits while the third has an approach line.', '两狼可以控制出口，第三狼仍有接近路线。', 'All three wolves crowd the same route.', '三只狼挤进同一条路线，彼此失去接应。', 'Separate containment, support, and capture roles.', '把控口、接应和捕食分给不同的狼。'),
  S('induced_movement', 'Induced movement', '诱导移动', 'Use a setup move to draw sheep into a useful lane.', '先走准备路线，逼羊进入目标区域。', 'A direct chase is blocked, but the flock has a predictable response route.', '直接追逐被挡住，但羊群存在可预测的回应路线。', 'Repeating the same chase and expecting the flock to open itself.', '重复同一条追逐路线，等待羊群自行露出破口。', 'Change the flock response before committing to capture.', '先改变羊群回应方向，再投入捕食。'),
