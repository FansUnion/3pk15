...
describe('player reproduction bundle', () => {
  it('keeps the board, AI seed progression, actions, and exact terminal reason', () => {
    ...
    const bundle = buildPlayerReproductionBundle({ state, difficulty: 'normal', aiProfile: 'tactical', initialAiSeed: 41, nextAiSeed: 44, actions: [{ type: 'pass' }], muted: false })
    ...
    expect(bundle).toMatchObject({ schema: 2, rulesVersion: 3, aiProfile: 'tactical', aiAlgorithmVersion: 'sheep-ai-v2' })
  })
})
