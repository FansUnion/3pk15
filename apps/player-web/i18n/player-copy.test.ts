...
describe('player-facing copy contract', () => {
  ...
  it('keeps locked access, help diagrams, machine links and ads on their intended surfaces', () => {
    ...
    const huntGate = readFileSync(resolve(process.cwd(), 'components/HuntAccessGate.tsx'), 'utf8')
    const privacy = readFileSync(resolve(process.cwd(), 'app/privacy/page.tsx'), 'utf8')
    const quests = readFileSync(resolve(process.cwd(), 'app/quests/page.tsx'), 'utf8')
    const homeQuest = readFileSync(resolve(process.cwd(), 'components/HomeQuestLink.tsx'), 'utf8')

    expect(gate).toContain('isLevelUnlocked')
    expect(list).toContain('isLevelUnlocked')
    expect(help).toContain('<BoardSvg')
    expect(help.match(/<RuleBoard/g)?.length).toBeGreaterThanOrEqual(5)
    expect(huntGate).toContain('isLevelUnlocked')
    for (const href of ['/chapters', '/skins', '/quests', '/settings', '/how-to-play']) expect(play).toContain(`href="${href}"`)
    expect(footer).toContain('<details')
    expect(footer).toContain('href="/skins"')
    expect(footer).toContain('href="/quests"')
    expect(footer).toContain('/llms.txt')
    expect(play).toContain('isRewardedAvailable')
    expect(play).toContain('nextUniversalSkinTarget')
    expect(play).toContain('rewardAdTarget')
    expect(play).toContain('claimableQuestCount')
    expect(play).toContain('questReady')
    expect(quests).toContain('claimAllQuests')
    expect(quests).toContain('targetMessage')
    expect(homeQuest).toContain('claimableQuestCount')
    expect(play).toContain('function ScopedLink')
    expect(play).not.toContain('grid grid-cols-3 items-center')
    expect(play).not.toContain("NEXT_PUBLIC_ADS_PROVIDER !== 'none'")
    expect(privacy).toContain("NEXT_PUBLIC_TELEMETRY === '1'")
  })
})
